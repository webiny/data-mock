...
| `project_environments` | project_id FK, env, variant, region, deployed, api_url, admin_url, api_token (encrypted), tenant, last_synced_at, archived_at | One Pulumi stack name. `api_token` is the main key: it only ever speaks for `tenant` (the default, root tenant), never for another. `variant` is `""` not NULL — SQLite treats NULLs as distinct in unique indexes. `deployed` means any app is deployed; `api_url` is null when the api app is not. `archived_at` soft-deletes it while keeping its slot in the unique index. |
...
| `project_tenants` | project_id FK, environment_id FK, tenant_id, name, api_token (encrypted) | Discovered tenants per environment. `api_token` is the tenant's own key and wins over the environment's; a re-pull carries it over. A non-default tenant with none cannot be pulled from or seeded. `EnvironmentContextService` resolves which token to send. |
| `project_groups` | project_id FK, environment_id FK, tenant, slug, name, remote_id | CMS content model groups, per tenant |
| `project_models` | project_id FK, environment_id FK, tenant, model_id, singular_api_name, plural_api_name, group_slug, plugin, fields (JSON) | CMS models + field definitions, per tenant. Pull Models pulls every tenant that has a key in one job; a tenant that fails is reported and the others still land. |
| `jobs` | project_id FK (nullable), environment_id FK (nullable), type, status, config (JSON), logs, **result (JSON)**, progress, progress_label | Background jobs in three scopes: global (neither id, e.g. pull-picsum, sync-preview), project (project only, e.g. sync-system) and environment (both, e.g. seed). `result` is the job's own answer, written through `context.setResult` — for jobs whose point is what they return rather than what they write. `logs` is never selected by a list: a deploy streams thousands of Pulumi lines into it, so the job detail route is the only thing that reads it. |
...
| `seed_jobs` | project_id FK, environment_id FK, status, config (JSON), result (JSON) | Per-seed-run record. `status` includes `cancelled`, and `config` carries the tenant and batch size so a run that stopped early can be resumed from what it did not create. |
