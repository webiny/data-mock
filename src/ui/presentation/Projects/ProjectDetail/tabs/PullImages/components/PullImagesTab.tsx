...
import type { ISyncLogVM } from "../abstractions/PullImagesTabPresenter.js";
...
export const PullImagesTab = observer(function PullImagesTab({ context }: PullImagesTabProps) {
  const { presenter } = useFeature(PullImagesTabFeature);
  ...
  return (
    <Stack gap="md">
      <Group justify="space-between">
        <Button loading={vm.isPullingFiles} onClick={() => presenter.pullFiles()}>
          Pull Files from FM
        </Button>
        <Text size="sm" c="dimmed">
          {vm.syncLogs.length} log{vm.syncLogs.length === 1 ? "" : "s"}
        </Text>
      </Group>
      <Text size="sm" c="dimmed">
