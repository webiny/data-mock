...
class GraphQLClientImpl implements Abstraction.Interface {
    ...
    try {
      const response = await pRetry(runQuery, {
        ...
        onFailedAttempt: ({ error }) => {
          this.logger.warn(`Failed attempt to execute query: ${error.message}.`);
        },
      });
  ...
  public async mutation<T>(
    ...
    try {
      const response = await pRetry(runMutation, {
        ...
        minTimeout: this.retryMinTimeout,
        onFailedAttempt: ({ error }) => {
          this.logger.warn(`Failed attempt to execute mutation: ${error.message}.`);
        },
      });
      return this.parse(response, getResult);
    } catch (err) {
  ...
  public async mutations<T>(
    ...
    this.logger.debug(`Total batches to execute: ${chunks.length}.`);

    for (let i = 0; i < chunks.length; i++) {
      this.logger.debug(`Executing batch ${i + 1} of ${chunks.length}...`);
...
export const GraphQLClient = Abstraction.createImplementation({
  ...
  dependencies: [HttpClient, GraphQLConfig, Logger],
});
